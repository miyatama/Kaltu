...
/**
 * Created by nmiya on 2018/01/01.
 */

public class CameraStateMachine {
    private static final String TAG = CameraStateMachine.class.getSimpleName();
    ...
    private Integer mCameraLens;

    public void open(Activity activity, AutoFitTextureView textureView, Integer CameraLens) {
        if (mState != null) throw new IllegalStateException("Alrady started state=" + mState);
        mTextureView = textureView;
        mCameraLens = CameraLens;
    ...
    public boolean takePicture(ImageReader.OnImageAvailableListener listener) {
        ...
        if (mCameraLens == CameraCharacteristics.LENS_FACING_FRONT){
    ...
    private final State mOpenCameraState = new State("OpenCamera") {
        public void enter() throws CameraAccessException {
            try{
                String cameraId = CameraUtil.getCameraId(mCameraManager, mCameraLens);
                CameraCharacteristics characteristics = mCameraManager.getCameraCharacteristics(cameraId);
                StreamConfigurationMap map = characteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);

                mImageReader = CameraUtil.getMaxSizeImageReader(map, ImageFormat.JPEG);
                Size previewSize = CameraUtil.getBestPreviewSize(map, mImageReader);
